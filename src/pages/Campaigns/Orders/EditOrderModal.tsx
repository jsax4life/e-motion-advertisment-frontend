...
interface Campaign {
  ...
  cherry_picked_dates?: string[];
  date_selection_mode?: "exclude" | "cherry-pick";
...
const OrderEditingModal: React.FC<OrderCreationModalProps> = ({
  ...
  useEffect(() => {
    if (orderToEdit) {
      ...
      // Initialize date selection mode and data
      if (orderToEdit.date_selection_mode) {
        setDateSelectionMode(orderToEdit.date_selection_mode);
      }
      if (orderToEdit.excluded_days) {
        setExcludedDays(orderToEdit.excluded_days);
      }
      if (orderToEdit.cherry_picked_dates) {
        setCherryPickedDates(orderToEdit.cherry_picked_dates);
      }
  ...
  const getCalendarData = () => {
    const startDate = orderDetails.campaign_start_date;
    const endDate = orderDetails.campaign_end_date;
              ...
              <div className="col-span-12">
                ...
                {dateSelectionMode === "cherry-pick" && (
                  <div className="p-4 border border-slate-200 rounded-lg bg-slate-50">
                    {!orderDetails.campaign_start_date || !orderDetails.campaign_end_date ? (
                      <p className="text-xs text-slate-500 text-center py-4">
                        Campaign dates are required to cherry-pick dates
                      </p>
                    ) : (
                      <>
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-200">
                            ...
                            <div className="flex gap-2">
                              <button
                                type="button"
                                onClick={() => {
                                  const allDates = getDatesInRange(orderDetails.campaign_start_date, orderDetails.campaign_end_date);
